...
{
	#Fixed GH likelihood method for logistic regression with random intercept
	nSim <- nrow(xwFSL)
	u <- xwFSL[, 1] * sqrt(2)
	...
	for(iter in 1:maxiter) {
		...
		iH <- solve(H1)
		delta <- iH %*% der
		theta.new <- theta + delta
		if(loglik.old > loglik) {
			for(try in 0:10) {
				loglik <- 0
				theta.new <- theta + delta/2^try
				b.new <- theta.new[1:m]
				sigma.new <- theta.new[m1]
				for(id in unique(di)) {
					yi <- y[di == id]
					ni <- length(yi)
					ee <- rep(1, ni)
					...
					Xib <- Xi %*% b.new
					bri <- sum(ri * b.new)
					sl <- es %*% t(Xib) + (sigma.new * u) %*% t(ee)
					esl <- exp(sl)
					ei <- (as.vector(exp(bri + sigma.new * ki * u - log(1 + esl) %*% ee))) * wFSL
					gi <- sum(ei)
					loglik <- loglik + log(gi)
				}
